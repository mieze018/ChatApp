...
自分の強みは、UI の不便さへの敏感さだと考えています。そのため、期限内で可能な限り直感的な操作性を高める工夫をしました。
自分で使ってみてちょっとでも気になるところは時間の許す限り調整しました。
たとえば、
...
- 現状入力フォームの操作性まで手が回らなかったので、react-hook-form などを導入し、キーボード操作などのアクセシビリティを高めることを第一に拡張したいです。
- 新規メッセージを受信すると、強制的に最新のメッセージまでスクロールするようにしましたが、古いメッセージを読んでいる時に困る挙動になるので、新規メッセージを受信時はその位置までスクロールできるボタンを表示したいです
- アイコンのみのボタンにはツールチップを表示したい
- サインアウト時は確認モーダルを表示したい、将来的にはメニューが増えていくことが想定されるので、ドロップダウンコンポーネントの作成が必要になりそうです
